...
crop_labels = np.clip(crop_labels, 0, 1000).astype(np.int32)
damage_labels = np.clip(damage_labels, 0, 1).astype(np.float32)
yield_labels = yield_labels.astype(np.float32)
...
tabular_data = tabular_df[tabular_columns].values.astype(np.float32)
